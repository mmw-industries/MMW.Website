import ProductDetailTemplate from "../../src/components/ProductDetailTemplate";

const meta = {
    title: "Gasanalysator OXYBABY® 6.0",
    description: "O2- bzw. O2/CO2-Gasanalysator für MAP-Verpackungen, Premiumversion",
    keywords: "Oxybaby, Oxybaby 6.0, Gasanalysator, O2, CO2, mobile",
    og_image: "https://www.wittgas.com/fileadmin/_processed_/0/a/csm_witt_gas_analyser_oxybaby_6_f07441a31f.png",
    og_url: "https://mmw.industries/products/oxybaby-6",
};

const data = {
    breadCrumb: "Startseite / Produkte / OXYBABY® 6.0",
    productTitle: "Gasanalysator OXYBABY® 6.0",
    overviewHeader: "O2- bzw. O2/CO2-Gasanalysator für MAP-Verpackungen, Premiumversion",
    overviewContent: [
        {

            type: 'list',
            header: "",
            items: [
                "deal zur Qualitätskontrolle\n" ,
                "schnelle, exakte Stichprobenanalyse\n" ,
                "misst auch den Gasdruck in der Verpackung\n" ,
                "geringer Messgasbedarf (2 ml)\n" ,
                "Anzeige des O2-Wertes in 0,01%-Schritten\n" ,
                "lange Lebensdauer des O2-Sensors (ca. 2 Jahre, je nach Nutzung)\n" ,
                "großer Messdatenspeicher mit Exportfunktion\n" ,
                "als Tischgerät verwendbar durch einfaches Einfügen eines Schlauches\n" ,
                "optional: Software für Datenmanagement und -analyse, Bluetooth, Barcode-Reader etc.",
            ]
        },
    ],
    productImages: [
        "https://www.wittgas.com/fileadmin/_processed_/0/a/csm_witt_gas_analyser_oxybaby_6_f07441a31f.png",
        "https://www.wittgas.com/fileadmin/_processed_/0/6/csm_witt_gas_analyser_oxybaby_6_table_top_4b47c645d8.png",
        "https://www.wittgas.com/fileadmin/_processed_/5/4/csm_Mensch_vor_PC_mit_OBCC_2_EN_124426dff4.jpg",
        "https://www.wittgas.com/fileadmin/_processed_/7/e/csm_carrying_case_for_witt_oxybaby_c00b3f3e3d.png",
    ],
    datasheetLink: "https://www.wittgas.com/fileadmin/user_upload/Dateien/data_sheets/german/gas_control/gasanalysator_oxybaby6_0_kpl_d.pdf",
    youtubeVideo: "<iframe width=\"852\" height=\"479\" src=\"https://www.youtube.com/embed/75dlhC9xmXI\" title=\"YouTube video player\" frameborder=\"0\" allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen></iframe>",
    informationHeader: "Schnurloser Ein-Hand-Analysator für die vorbeugende Haltbarkeitskontrolle schutzgasverpackter Lebensmittel (MAP). Das ideale Instrument für mobile, schnelle und exakte Stichproben direkt an der Verpackungsmaschine, im Lager oder im Labor.",
    informationContent: [
        "OXYBABY® 6.0, die preiswerte Alternative zu stationären Messgeräten in der Lebensmittel- und Schweißtechnik. Optional bietet es schnell und unkompliziert alle Vorteile modernster Technologien z.B.: Bluetooth zur drahtlosen Kommunikation und einen integrierten Barcode-Reader.",
        "Durch die mobile Verpackungskontrolle sichern Sie eine gleichbleibende Frischequalität Ihrer Produkte (HACCP) für dauerhafte Lieferbeziehungen.\n" +
        "Das ideale Messgerät für die Verpackungsüberwachung – mit geringstem Messgasbedarf für kleinste Verpackungen in der Schutzbegasung. Eine Druckanzeige informiert über den Innendruck der Verpackung.\n" +
        "Durch die eigens entwickelte OBCC-Software dokumentieren Sie Ihrem Kunden die Messergebnisse und garantieren damit höchste Verpackungsqualität und optimale Frische.",
        {
            type: 'list',
            header: "Vorteile:",
            items: [
                "minimaler Messgasbedarf für Kleinstverpackungen\n",
                "integrierter Messwertspeicher für die letzten 500 Messungen\n",
                "Produktverwaltung von bis zu 25 Anwendern und Zuordnung der Messwerte für bis zu 100 Produktnamen und 50 Verpackungslinien\n",
                "eine microSD-Karte ermöglicht den leichten Export der Messergebnisse und vergrößert die Speicherkapazität\n",
                "Druckanzeige alternativ wählbar\n",
                "schnurlos mobil durch Akkubetrieb\n",
                "USB-Port für Datentransfer und Akkuladung\n",
                "optional integrierter Barcode-Reader zum Einlesen der Produktdaten für schnelleren Work-Flow\n",
                "keine Fehlmessungen durch Durchflusskontrolle mit Warnhinweis bei verstopfter Nadel\n",
                "verbesserte einfache Einhandbedienung\n",
                "alternativ Messung auch mit Schlauch möglich\n",
                "leicht zu reinigende Kunststoffoberfläche\n",
                "großes, gut ablesbares, beleuchtetes Grafikdisplay\n",
                "mehrsprachige Ausführung\n",
                "integrierte Nadelabdeckung zum Schutz des Bedieners\n",
                "lange Lebensdauer des O2-Sensors (ca. 2 Jahre)\n",
                "geringe Wartungskosten"
            ]
        },
        {
            type: 'list',
            header: "Lieferumfang komplett im handlichen Koffer inklusive:",
            items: [
                "Ladegerät, microSD-Karte\n" ,
                "Link zu OBCC-Software (Demoversion)\n" ,
                "2 Ersatznadeln\n" ,
                "2 Ersatzfilter\n" ,
                "100 selbstklebende Dichtplättchen"
            ],
        },
        {
            type: 'span',
            content: "Abmessungen Koffer (HxBxT): ca. 325x385x115 mm,\n" +
                "Gewicht: ca. 1,7 kg (inkl. Inhalt)"
        },
        {
            type: 'list',
            header: "Optionen:",
            items: [
                "Schlauch mit Luer-Lok-Verbindung zur stationären Messung\n" ,
                "Datenkabel\n" ,
                "integrierter Barcode-Scanner\n" ,
                "Bluetooth (z.B. für separaten Tischdrucker)"
            ]
        }
    ],
    downloads: [
        {
            image: "https://www.wittgas.com/fileadmin/_processed_/0/1/csm_gasanalysator_oxybaby6_0_kpl_d_203224f23f.png",
            link: "https://www.wittgas.com/fileadmin/user_upload/Dateien/data_sheets/german/gas_control/gasanalysator_oxybaby6_0_kpl_d.pdf",
            title: "Datenblatt",
            content: "Alle Technischen Informationen zu dem Produkt"
        },
        {
            image: "https://www.wittgas.com/fileadmin/_processed_/4/0/csm_MAP_Gasanalyse_D_6af455217b.png",
            link: "https://www.wittgas.com/fileadmin/user_upload/Dateien/brochures/german/MAP_Gasanalyse_D.pdf",
            title: "Broschüre WITT Gasanalysatoren",
            content: ""
        },
        {
            image: "https://www.wittgas.com/fileadmin/_processed_/4/1/csm_MAP_D_f6b462f88e.png",
            link: "https://www.wittgas.com/fileadmin/user_upload/Dateien/brochures/german/MAP_D.pdf",
            title: "Broschüre WITT Produkte für MAP",
            content: ""
        },
        {
            image: "https://www.wittgas.com/fileadmin/_processed_/5/7/csm_MAP_Obst_Gemuese_D_def4e74d64.png",
            link: "https://www.wittgas.com/fileadmin/user_upload/Dateien/brochures/german/MAP_Obst_Gemuese_D.pdf",
            title: "Broschüre MAP Obst & Gemüse",
            content: ""
        }
    ]
};

function Oxybaby6() {
    return (
        <ProductDetailTemplate
            data={data}
            meta={meta}
        />
    )
}

export default Oxybaby6;